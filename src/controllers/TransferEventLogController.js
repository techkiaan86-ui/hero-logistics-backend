...
// Get all TransferEventLogs with pagination, sorting and filtering
exports.getAll = async (req, res, next) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.transferEventLog.findMany({
        where, skip, take, orderBy
      }),
      prisma.transferEventLog.count({ where })
    ]);
...
exports.getById = async (req, res, next) => {
  try {
    ...
    const data = await prisma.transferEventLog.findFirst({ where });
    
    if (!data) {
      return sendError(res, {
        code: ERROR_CODES.NOT_FOUND,
        message: 'TransferEventLog not found'
...
// Update TransferEventLog with Optimistic Concurrency check
...
exports.delete = async (req, res, next) => {
  try {
    ...
    await prisma.transferEventLog.delete({ where });
