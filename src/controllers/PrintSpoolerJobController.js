...
// Get all PrintSpoolerJobs with pagination, sorting and filtering
exports.getAll = async (req, res, next) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.printSpoolerJob.findMany({
        where, skip, take, orderBy
      }),
      prisma.printSpoolerJob.count({ where })
    ]);
...
exports.getById = async (req, res, next) => {
  try {
    ...
    const data = await prisma.printSpoolerJob.findFirst({ where });
    
    if (!data) {
      return sendError(res, {
        code: ERROR_CODES.NOT_FOUND,
        message: 'PrintSpoolerJob not found'
...
// Update PrintSpoolerJob with Optimistic Concurrency check
...
exports.delete = async (req, res, next) => {
  try {
    ...
    await prisma.printSpoolerJob.delete({ where });
