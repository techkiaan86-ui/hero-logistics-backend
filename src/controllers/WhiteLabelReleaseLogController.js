...
// Get all WhiteLabelReleaseLogs with pagination, sorting and filtering
exports.getAll = async (req, res, next) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.whiteLabelReleaseLog.findMany({
        where, skip, take, orderBy
      }),
      prisma.whiteLabelReleaseLog.count({ where })
    ]);
...
exports.getById = async (req, res, next) => {
  try {
    ...
    const data = await prisma.whiteLabelReleaseLog.findFirst({ where });
    
    if (!data) {
      return sendError(res, {
        code: ERROR_CODES.NOT_FOUND,
        message: 'WhiteLabelReleaseLog not found'
...
// Update WhiteLabelReleaseLog with Optimistic Concurrency check
...
exports.delete = async (req, res, next) => {
  try {
    ...
    await prisma.whiteLabelReleaseLog.delete({ where });
