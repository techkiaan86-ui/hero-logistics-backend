...
exports.getById = async (req, res, next) => {
  try {
    ...
    if (req.user?.role !== 'SUPER_ADMIN') {
      if (!companyId) {
        return sendError(res, { code: ERROR_CODES.NOT_FOUND, message: 'AuditLog not found' }, HTTP_STATUS.NOT_FOUND);
      }
      where.companyId = companyId;
    }
...
// Update AuditLog with tenant ownership check
exports.update = async (req, res, next) => {
  try {
    ...
    const existing = await prisma.auditLog.findFirst({ where: findWhere });
    if (!existing) {
      return sendError(res, { code: ERROR_CODES.NOT_FOUND, message: 'AuditLog not found' }, HTTP_STATUS.NOT_FOUND);
    }
...
// Delete AuditLog with tenant ownership check
