...
// Get all NetworkedPrinters with pagination, sorting and filtering
exports.getAll = async (req, res, next) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.networkedPrinter.findMany({
        where, skip, take, orderBy
      }),
      prisma.networkedPrinter.count({ where })
    ]);
...
exports.getById = async (req, res, next) => {
  try {
    ...
    const data = await prisma.networkedPrinter.findFirst({ where });
    
    if (!data) {
      return sendError(res, {
        code: ERROR_CODES.NOT_FOUND,
        message: 'NetworkedPrinter not found'
...
// Update NetworkedPrinter with Optimistic Concurrency check
...
exports.delete = async (req, res, next) => {
  try {
    ...
    await prisma.networkedPrinter.delete({ where });
