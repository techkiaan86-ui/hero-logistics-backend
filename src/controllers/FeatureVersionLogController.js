...
// Get all FeatureVersionLogs with pagination, sorting and filtering
exports.getAll = async (req, res, next) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.featureVersionLog.findMany({
        where, skip, take, orderBy
      }),
      prisma.featureVersionLog.count({ where })
    ]);
...
exports.getById = async (req, res, next) => {
  try {
    ...
    const data = await prisma.featureVersionLog.findFirst({ where });
    
    if (!data) {
      return sendError(res, {
        code: ERROR_CODES.NOT_FOUND,
        message: 'FeatureVersionLog not found'
...
// Update FeatureVersionLog with Optimistic Concurrency check
...
exports.delete = async (req, res, next) => {
  try {
    ...
    await prisma.featureVersionLog.delete({ where });
