...
// Get all EquipmentSwaps with pagination, sorting and filtering
exports.getAll = async (req, res, next) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.equipmentSwap.findMany({
        where, skip, take, orderBy
      }),
      prisma.equipmentSwap.count({ where })
    ]);
...
exports.getById = async (req, res, next) => {
  try {
    ...
    const data = await prisma.equipmentSwap.findFirst({ where });
    
    if (!data) {
      return sendError(res, {
        code: ERROR_CODES.NOT_FOUND,
        message: 'EquipmentSwap not found'
...
// Update EquipmentSwap with Optimistic Concurrency check
...
exports.delete = async (req, res, next) => {
  try {
    ...
    await prisma.equipmentSwap.delete({ where });
