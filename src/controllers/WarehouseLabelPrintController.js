...
// Get all WarehouseLabelPrints with pagination, sorting and filtering
exports.getAll = async (req, res, next) => {
  try {
    ...
    const [data, total] = await Promise.all([
      prisma.warehouseLabelPrint.findMany({
        where, skip, take, orderBy
      }),
      prisma.warehouseLabelPrint.count({ where })
    ]);
...
exports.getById = async (req, res, next) => {
  try {
    ...
    const data = await prisma.warehouseLabelPrint.findFirst({ where });
    
    if (!data) {
      return sendError(res, {
        code: ERROR_CODES.NOT_FOUND,
        message: 'WarehouseLabelPrint not found'
...
// Update WarehouseLabelPrint with Optimistic Concurrency check
...
exports.delete = async (req, res, next) => {
  try {
    ...
    await prisma.warehouseLabelPrint.delete({ where });
