...
// Get all CompanyFeatureOverrides with pagination, sorting and filtering
exports.getAll = async (req, res, next) => {
  try {
    ...
    const [data, total] = await Promise.all([
      ...
      prisma.companyFeatureOverride.count({ where })
    ]);
...
exports.getById = async (req, res, next) => {
  try {
    ...
    const data = await prisma.companyFeatureOverride.findFirst({ where });
    
    if (!data) {
      return sendError(res, {
        code: ERROR_CODES.NOT_FOUND,
        message: 'CompanyFeatureOverride not found'
...
// Update CompanyFeatureOverride with Optimistic Concurrency check
...
exports.delete = async (req, res, next) => {
  try {
    ...
    await prisma.companyFeatureOverride.delete({ where });
